 ...
 *TA0_4: P2.7  pwm功能输出
 *
 *TIMER_A1_BASE:   PWM CONFIGURE  7.7输出（代码注释）
 *TIMER_A1_BASE：  超声波模块  （ 现在选用）
 *TIMER_A2_BASE：  TA2_0用于计算速度
 *TIMER_A3_BASE :       连续计数模式  &   a相上升沿捕获
 *TA3_0_IRQHandler      TA3_0  右轮 a相:P10.4 //判断b相:P10.5
 *TA3_N_IRQHandler    ：TA3_2：左轮 a相:P8.2 // 判断b相:P9.2
 *
 *
 *
 *
...
void TimA2_Cap_Init(void)
{
    ...
    MAP_Timer_A_clearCaptureCompareInterrupt(TIMER_A2_BASE, CAP_REGISTER_SELECTION); //清除 CCR1 更新中断标志位
...
void TA2_N_IRQHandler(void)
{
    ...
    {
        if (MAP_Timer_A_getEnabledInterruptStatus(TIMER_A2_BASE)) //溢出中断
        {
            MAP_Timer_A_clearInterruptFlag(TIMER_A2_BASE); //清除定时器溢出中断标志位

            /* ★ 软件复位COV ★ */
            /* 这里UP忘记讲了，如果在未清除中断位值时，来了一次中断，COV会置位，需要软件复位，这里没有官方库函数。具体可以参考技术手册(slau356h.pdf) P790 */
            BITBAND_PERI(TIMER_A_CMSIS(TIMER_A2_BASE)->CCTL[CAP_CCR_NUM], TIMER_A_CCTLN_COV_OFS) = 0;
        ...
        if (MAP_Timer_A_getCaptureCompareEnabledInterruptStatus(TIMER_A2_BASE, CAP_REGISTER_SELECTION)) //捕获中断
        {
            MAP_Timer_A_clearCaptureCompareInterrupt(TIMER_A2_BASE, CAP_REGISTER_SELECTION); //清除 CCR1 更新中断标志位

            //判断是否捕获到下降沿
            if (TIMA2_CAP_STA & 0X40 && (MAP_Timer_A_getSynchronizedCaptureCompareInput(TIMER_A2_BASE,
                                                                                        CAP_REGISTER_SELECTION,
                                                                                        TIMER_A_READ_CAPTURE_COMPARE_INPUT) == TIMER_A_CAPTURECOMPARE_INPUT_LOW))
            {
                TIMA2_CAP_STA |= 0X80; //标记成功捕获完高电平
                TIMA2_CAP_VAL = Timer_A_getCaptureCompareCount(TIMER_A2_BASE, CAP_REGISTER_SELECTION);
            }
            else //还未开始,第一次捕获上升沿
            {
                TIMA2_CAP_STA = 0;
                TIMA2_CAP_VAL = 0;
                MAP_Timer_A_clearTimer(TIMER_A2_BASE); //清空定时器 重新从0计数
